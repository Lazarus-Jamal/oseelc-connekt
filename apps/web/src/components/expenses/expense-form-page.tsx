...
import { EXPENSE_CATEGORIES, formatCurrency, formatFileSize } from '@care-connekt/shared'

const DEFAULT_EXPENSE_CATS = EXPENSE_CATEGORIES as readonly string[]

const itemSchema = z.object({
  label: z.string().min(1, 'Libellé requis'),
  category: z.string().min(1, 'Catégorie requise'),
  amount: z.number({ invalid_type_error: 'Montant invalide' }).nonnegative(),
...
const schema = z.object({
  ...
  periodStart: z.string().min(1, 'Date requise'),
  periodEnd: z.string().min(1, 'Date requise'),
...
export function ExpenseFormPage() {
  ...
  const addCategory = async () => {
    ...
    const res = await fetch('/api/categories', {
      ...
      body: JSON.stringify({ name, declarationType: 'EXPENSE' }),
    })
  ...
  const addFile = (file: File) => {
    if (!ALLOWED_TYPES.includes(file.type)) { toast.error('Type non autorisé'); return }
    if (file.size > MAX_SIZE) { toast.error('Fichier trop volumineux (max 10 Mo)'); return }
    if (files.some((f) => f.name === file.name && f.size === file.size)) { toast.error('Fichier déjà ajouté'); return }
  ...
  return (
    <div className="max-w-4xl">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        {/* Soumettant */}
        <div className="bg-orange-50 dark:bg-orange-900/20 rounded-xl border border-orange-100 dark:border-orange-800 px-5 py-3 flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-orange-800 dark:text-orange-200">{session?.user?.name}</p>
            <p className="text-xs text-orange-600 dark:text-orange-400">{session?.user?.email}</p>
          </div>
          <span className="ml-auto text-xs font-medium px-2.5 py-1 rounded-full bg-orange-100 dark:bg-orange-800 text-orange-700 dark:text-orange-300">
            Soumettant
          </span>
        </div>
        ...
        {!session?.user?.facilityId && (
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
            ...
            <select
              ...
              className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-500"
            >
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="date" max={today} {...register('periodStart')}
                className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-500" />
            ...
            {periodType !== 'DAILY' && (
              <div>
                ...
                <input type="date" max={today} {...register('periodEnd')}
                  className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-500" />
              </div>
            )}
          </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          <div className="flex items-center justify-between mb-4">
            <h3 className="font-semibold text-gray-900 dark:text-white">Lignes de dépenses</h3>
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={() => append({ label: '', category: categories[0] || '', amount: 0, note: '' })}
          ...
          {showAddCat && (
            <div className="mb-3 flex gap-2 p-3 bg-orange-50 dark:bg-orange-900/20 rounded-xl border border-orange-100 dark:border-orange-800">
              <input type="text" value={newCatName} onChange={(e) => setNewCatName(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addCategory())}
                placeholder="Nom de la catégorie…"
                className="flex-1 px-3 py-1.5 text-sm border border-orange-200 dark:border-orange-700 rounded-lg bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-400" />
              <button type="button" onClick={addCategory} disabled={addingCat || !newCatName.trim()}
                className="inline-flex items-center gap-1 px-3 py-1.5 bg-orange-500 hover:bg-orange-600 text-white text-xs font-medium rounded-lg disabled:opacity-50 transition">
          ...
          <div className="space-y-4">
            {fields.map((field, index) => (
              <div key={field.id} className="p-4 bg-orange-50/50 dark:bg-orange-900/10 rounded-xl border border-orange-100 dark:border-orange-800/30">
                <div className="grid grid-cols-12 gap-3 items-start">
                  <div className="col-span-5">
                    <label className="block text-xs font-medium text-gray-500 mb-1">Libellé <span className="text-red-400">*</span></label>
                    <input {...register(`items.${index}.label`)} placeholder="Ex : Achat médicaments"
                      className="w-full px-2.5 py-1.5 border border-gray-200 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                    {errors.items?.[index]?.label && <p className="mt-0.5 text-xs text-red-500">{errors.items[index]?.label?.message}</p>}
                  </div>
                  ...
                  <div className="col-span-2">
                    ...
                    <input type="number" step="1" min="0" {...register(`items.${index}.amount`, { valueAsNumber: true })} placeholder="0"
                      className="w-full px-2.5 py-1.5 border border-gray-200 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-500" />
                  </div>
                ...
                <div className="mt-3">
                  <label className="block text-xs font-medium text-gray-500 mb-1">Notes sur cette ligne</label>
                  <textarea {...register(`items.${index}.note`)} rows={2} placeholder="Précisions, fournisseur, référence..."
                    className="w-full px-2.5 py-1.5 border border-gray-200 dark:border-gray-600 rounded-lg text-sm bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-500 resize-none" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="mt-4 flex items-center justify-end gap-3 border-t border-gray-200 dark:border-gray-700 pt-4">
            ...
            <span className="text-xl font-bold text-orange-600 dark:text-orange-400">{formatCurrency(totalAmount)}</span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          ...
          <textarea {...register('notes')} rows={4}
            placeholder="Remarques générales, contexte particulier, informations complémentaires..."
            className="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-500 resize-none" />
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5">
          ...
          <div onDrop={(e) => { e.preventDefault(); Array.from(e.dataTransfer.files).forEach(addFile) }}
            onDragOver={(e) => e.preventDefault()} onClick={() => fileInputRef.current?.click()}
            className="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl p-6 text-center cursor-pointer hover:border-orange-400 hover:bg-orange-50/50 dark:hover:bg-orange-900/10 transition group">
            ...
            <Upload className="w-8 h-8 text-gray-300 group-hover:text-orange-400 mx-auto mb-2 transition" />
            <p className="text-sm text-gray-500 group-hover:text-orange-600 transition">
              Glissez vos fichiers ici ou <span className="text-orange-600 font-medium">cliquez pour parcourir</span>
            </p>
          </div>
          {files.length > 0 && (
