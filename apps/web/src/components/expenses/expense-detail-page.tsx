...
export function ExpenseDetailPage({ id }: ExpenseDetailPageProps) {
  ...
  if (!declaration) {
    return (
      <div className="text-center py-16">
        ...
        <Link href="/expenses" className="mt-2 text-orange-600 hover:underline text-sm">Retour à la liste</Link>
      </div>
    )
  }
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-800">
              <h2 className="font-semibold text-gray-900 dark:text-white">Détail des dépenses</h2>
            </div>
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                {declaration.items.map((item: any) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-4 py-2.5 text-right font-semibold text-orange-600 dark:text-orange-400">
                      {formatCurrency(item.amount)}
                    </td>
                  </tr>
                ))}
                <tr className="bg-orange-50 dark:bg-orange-900/10 font-semibold">
                  <td className="px-4 py-3 text-gray-900 dark:text-white" colSpan={2}>Total des dépenses</td>
                  <td className="px-4 py-3 text-right text-orange-600 dark:text-orange-400 text-base">{formatCurrency(declaration.totalAmount)}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          {declaration.comment && declaration.status !== 'REJECTED' && (
            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1">Observations générales</p>
              <p className="text-sm text-gray-700 dark:text-gray-300">{declaration.comment}</p>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl w-full max-w-md p-6">
            ...
            <textarea
              ...
              className="w-full px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm bg-white dark:bg-gray-800 focus:outline-none focus:ring-2 focus:ring-orange-500 resize-none"
            />
